...
export default (function (db, _ref) {
    ...
    return db.commands.filter(function (command) {
        ...
        return [].concat(_toConsumableArray(acc), _toConsumableArray(command.basket.filter(function () {
            return weightedBoolean(40);
        }) // reviewers review 40% of their products
        .map(function (product) {
            var date = randomDate(command.date);
            var status = isAfter(aMonthAgo, date) ? weightedArrayElement(['accepted', 'rejected'], [3, 1]) : weightedArrayElement(['pending', 'accepted', 'rejected'], [5, 3, 1]);
            ...
            return {
                ...
                comment: Array.apply(null, Array(random.number({ min: 1, max: 5 }))).map(function () {
