...
export default (function (db, _ref) {
    ...
    return Array.from(Array(600).keys()).map(function (id) {
        ...
        var customer = random.arrayElement(db.customers.filter(function (customer) {
            return customer.has_ordered;
        }));
        var date = randomDate(customer.first_seen, customer.last_seen);

        var status = isAfter(date, aMonthAgo) && random.boolean() ? 'ordered' : weightedArrayElement(['delivered', 'cancelled'], [10, 1]);
        return {
            id: id,
